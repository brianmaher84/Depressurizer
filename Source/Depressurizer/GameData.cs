...
namespace Depressurizer
{
	...
	public class GameList
	{
		...
		public static string FetchHtmlFromUrl(string url)
		{
			...
			{
				Program.Logger.Error(GlobalStrings.GameData_ExceptionDownloadHTMLGameList, e.Message);
		...
		public void ExportSteamConfigFile(string filePath, bool discardMissing)
		{
			...
			{
				Program.Logger.Error(GlobalStrings.GameData_ErrorSavingSteamConfigFile, e.ToString());
		...
		public void ExportSteamShortcuts(long SteamId)
		{
			...
			{
				Program.Logger.Error(GlobalStrings.GameData_ErrorOpeningConfigFileParam, e.ToString());
			}
			catch (IOException e)
			{
			...
			if (dataRoot != null)
			{
				...
				if (dataRoot.NodeType == ValueType.Array)
				{
					...
					{
						Program.Logger.Error(GlobalStrings.Log_GameData_ShortcutBackupFailed, e.Message);
					}
		...
		public int IntegrateHtmlGameList(string page, bool overWrite, SortedSet<int> ignore, out int newItems)
		{
			...
			Program.Logger.Info(GlobalStrings.GameData_IntegratedHTMLDataIntoGameList, totalItems, newItems);
			return totalItems;
		}
		...
		public int IntegrateXmlGameList(XmlDocument doc, bool overWrite, SortedSet<int> ignore, out int newItems)
		{
			...
			Program.Logger.Info(GlobalStrings.GameData_IntegratedXMLDataIntoGameList, loadedGames, newItems);
			return loadedGames;
		}
					...
					if (int.TryParse(gameNodePair.Key, out gameId))
					{
						...
						else if (gameNodePair.Value != null && gameNodePair.Value.NodeType == ValueType.Array)
						{
							...
							if (gameNodePair.Value.ContainsKey("LastPlayed") && gameNodePair.Value["LastPlayed"].NodeInt != 0)
							{
								...
								Program.Logger.Verbose(GlobalStrings.GameData_ProcessedGame, gameId, Utility.GetDTFromUTime(game.LastPlayed).Date);
							}
						}
					}
				}
		...
		private GameInfo IntegrateGame(int appId, string appName, bool overwriteName, SortedSet<int> ignore, GameListingSource src, out bool isNew)
		{
			...
			Program.Logger.Verbose(GlobalStrings.GameData_IntegratedGameIntoGameList, appId, appName, isNew);
			return result;
		}
				...
				foreach (KeyValuePair<string, VDFNode> gameNodePair in gameNodeArray)
				{
					...
					if (int.TryParse(gameNodePair.Key, out gameId))
					{
						...
						else if (gameNodePair.Value != null && gameNodePair.Value.NodeType == ValueType.Array)
						{
							...
							Program.Logger.Verbose(GlobalStrings.GameData_ProcessedGame, gameId, string.Join(",", game.Categories));
						}
					}
				}
			}
		...
		private bool RemoveGame(int appId)
		{
			...
			if (appId < 0)
			{
				if (Games.ContainsKey(appId))
				{
					...
					if (removed)
					{
						Program.Logger.Verbose(GlobalStrings.GameData_RemovedGameFromGameList, appId, removedGame.Name);
					}
					else
					{
						Program.Logger.Error(GlobalStrings.GameData_ErrorRemovingGame, appId, removedGame.Name);
					}
